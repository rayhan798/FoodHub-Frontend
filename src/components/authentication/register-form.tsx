...
interface ExtendedSignUpEmailOptions {
  name: string;
  email: string;
  password: string;
  role: string;
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      ...
      try {
        ...
        // ৩. অটোমেটিক রিডাইরেক্ট লজিক (১.৫ সেকেন্ড পর যাতে ইউজার সাকসেস টোস্ট দেখতে পায়)
        setTimeout(() => {
          router.push("/login");
        }, 1500);
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-start gap-2 p-2 rounded-md bg-slate-50 border border-slate-100">
                ...
                <p className="text-xs text-slate-500">
                  {role === Roles.PROVIDER
                    ? "Providers can list meals, manage menus, and receive orders from customers."
                    : "Customers can explore various meals, order food, and track deliveries."}
                </p>
              </div>
            </div>
