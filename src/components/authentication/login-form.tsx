...
import { Roles } from "@/constants/roles"; // নিশ্চিত করুন এই ফাইলটি আছে
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      ...
      try {
        ...
        // ✅ TypeScript 'role' এরর ফিক্স করা হয়েছে 'as any' ব্যবহারের মাধ্যমে
        const user = data?.user as any;
        const userRole = user?.role;

        toast.success("Logged in successfully!", { id: toastId });

        // 🎯 রোল অনুযায়ী অটোমেটিক রিডাইরেক্ট লজিক
        if (userRole === Roles.ADMIN) {
          router.push("/admin");
        } else if (userRole === Roles.PROVIDER) {
          router.push("/provider/dashboard");
        } else {
          router.push("/"); // Customer এর জন্য হোম পেজ
        }
